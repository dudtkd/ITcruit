...
@Data
public class ReviewPaginationVO<T> {
	private int totalRecord;	// 총 게시글 수
	private int totalPage;		// 총 페이지 수
	private int currentPage;	// 현재 페이지
	private int screenSize = 3;		// 페이지 당 게시글 수
	...
	private String reviewWorkStts;
	
	public ReviewPaginationVO() {}
	
	// PaginationInfoVO 객체를 만들 때, 한 페이지당 게시글 수와 페이지 블록 수를
	// 원하는 값으로 초기화 할 수 있다.
	
	public ReviewPaginationVO(int screenSize, int blockSize) {
		super();
	...
	public String getpagingHTML() {
		 ...
		 */
		if(startPage > 1) {
			html.append("<li class='page-item mb-0'><a href = '' class='page-link' data-reviewpage='"
		...
		if(endPage < totalPage) {
			html.append("<li class='page-item'><a href='' class='page-link' data-reviewpage='"

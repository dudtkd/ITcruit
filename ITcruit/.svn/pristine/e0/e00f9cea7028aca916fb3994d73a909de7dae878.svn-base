...
public class EnterpriseServiceImpl implements IMyEnterpriseService {
	...
	public ServiceResult recruitRegisterRemove(RecruitVO recruitVO, HttpServletRequest req) {
		...
		log.info(">>>>>>>>>>>"  + recruitVO.getPstnCmmncdNm());
		log.info(">>>>>>>>>>>"  + recruitVO.getRecruitNo());
		//기업 로그인 세션에서 꺼낸다.
		int entNo = 9999999; 
		...
		try {
			...
			if (pass > 0) {
				...
				//apply_process_list 삭제 recruitVO에 있는 recruitNo pstnCmmncdNm 로
				//RecruitVO recruitVO, String recruitNo, String pstnCmmncdNm
				pass = applyProcessList(recruitVO, recruitVO.getRecruitNo(), recruitVO.getPstnCmmncdNm()); //(recruitVO, req, entNo);
				if (pass == 0) return ServiceResult.FAILED;
				result = ServiceResult.OK;
	...
	public int preferlang(RecruitVO recruitVO) {
		...
		if(recruitVO.getFlag() == 1) {
			...
			if(recruitVO.getPreferLangCmmncdNms() != null && recruitVO.getPreferLangCmmncdNms().size() != 0) {
	...
	private int applyProcessList(RecruitVO recruitVO, String recruitNo, String pstnCmmncdNm) {
		...
		if(recruitVO.getFlag() == 1) {
			...
			try {
				...
				for(int i = 0; i < recruitVO.getOrder().length; i++) {
					ApplyProcessListVO aplv = null;
					
					if(i == 0) {
						
						aplv = new ApplyProcessListVO();
						...
						String recruitStartStr = recruitVO.getRecruitStrtDt();
						String recruitEndStr = recruitVO.getRecruitEndDt();
						
						Date strtYmd = isoFormat.parse(recruitStartStr);
					    Date prsntnYmd = isoFormat.parse(recruitEndStr);
						
						aplv.setStrtYmd(strtYmd);
						aplv.setPrsntnYmd(prsntnYmd);
					...
					if(recruitVO.getName()[i].equals("Coding test")) {
						
						EntCodingtestVO ecv = new EntCodingtestVO();
						
						ecv.setRecruitNo(recruitNo);
						ecv.setPstnCmmncdNm(pstnCmmncdNm);
						ecv.setProcessNo(aplv.getProcessSeq());
						ecv.setProcessNoOrder(recruitVO.getOrder()[i]);
						
						myEntMapper.entCodingTestInsert(ecv);
		...
		//삭제
		if(recruitVO.getFlag() == 3) {
			return myEntMapper.applyProcessListInsertDelete(recruitVO);
		} 
	...
	public ServiceResult qstnDSNCTInsert(QstnDSNCTVO qdv) {
		...
		int pass = myEntMapper.qstnDSNCTInsert(qdv);
	...
	public ServiceResult updateFinalApplicantPass(String eachProcessSeq) {
		...
		int pass = myEntMapper.updateFinalApplicantPass(eachProcessSeq);
...
//			
	...
	public int insertApplicantPass(UpdateApplicantVO uav) {
		return myEntMapper.insertApplicantPass(uav);
	}
	...
	public ServiceResult updateApplicantFail(String eachProcessSeq) {
		...
		int pass = myEntMapper.updateApplicantFail(eachProcessSeq);
	...
	public List<EntIntroQSTNVO> getEntCodingTestNo(String recruitNo, String pstnCmmncdNm) {
		return myEntMapper.getEntCodingTestNo(recruitNo, pstnCmmncdNm);
	}
	...
	public ServiceResult codingTestInsert(EntCodingTestQstnDsnctVO ectqdv) {
		...
		int pass = myEntMapper.codingTestInsert(ectqdv);
	...
	public List<EntCodingTestQstnDsnctVO> selectCondingTestOrder(String recruitNo, String pstnCmmncdNm,
			String orderNumber) {
		return myEntMapper.selectCondingTestOrder(recruitNo, pstnCmmncdNm, orderNumber);
	}
	...
	public List<AccountVO> selectCodingTesterList(CustomPaginationInfoVO<AccountVO> accountList) {
		return myEntMapper.selectCodingTesterList(accountList);
	}
	...
	public List<EntCodingtestVO> getcodingtestAnswer(EntCodingtestVO entcodingtestVO) {
		return myEntMapper.getcodingtestAnswer(entcodingtestVO);
	}
	...
	public Integer selectCodingTestLastNoOrder(int entCodingTestNo) {
		return myEntMapper.selectCodingTestLastNoOrder(entCodingTestNo);
	}
	...
	public List<AccountVO> getApplicantRtcList(CustomPaginationInfoVO<AccountVO> accountList) {
		return myEntMapper.getApplicantRtcList(accountList);
	}
	...
	public ApplyProcessListVO selectApplicantDate(Map<String, String> params) {
		return myEntMapper.selectApplicantDate(params);
	}

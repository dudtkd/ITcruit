...
@Data
public class ListPaginationVO<T> {
	private int totalRecord;	// 총 게시글 수
	private int totalPage;		// 총 페이지 수
	private int currentPage;	// 현재 페이지
	private int screenSize = 100;		// 페이지 당 게시글 수
	...
	private String loginId; // 로그인한 회원의 아이디
	private int entNo; // 기업 번호
	
	private String searchEnt; // 기업 이름 검색
	private String searchJob; // 직무 검색
	private String searchCount; // 직무 검색
	private String searchLoc; // 지역 검색
	
	public ListPaginationVO() {}
	
	// PaginationInfoVO 객체를 만들 때, 한 페이지당 게시글 수와 페이지 블록 수를
	// 원하는 값으로 초기화 할 수 있다.
	
	public ListPaginationVO(int screenSize, int blockSize) {
		super();
	...
	public String getpagingHTML() {
		...
		html.append("<nav class='mt-4 d-flex justify-content-center' aria-label='navigation'>");
		html.append("<ul class='pagination pagination-primary-soft d-inline-block d-md-flex rounded mb-0'>");
		 ...
		 */
		if(startPage > 1) {
			html.append("<li class='page-item mb-0'><a href = '' class='page-link' data-page='"

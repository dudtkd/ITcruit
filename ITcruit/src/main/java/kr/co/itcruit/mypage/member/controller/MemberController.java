...
//			boolean flag = (boolean) deleteData.get("flag");
	...
	public String talentCheck(Model model
			, @RequestParam(name = "page", required = false, defaultValue = "1") int currentPage,
			HttpServletRequest req,
			@RequestParam(required = false, name="alarmNo") String alarmNo) {
		
		CustomUser user = (CustomUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
	    String loginUser = (String) user.getAccount().getAccountId();
	    alarmService.setUpdateReadY(alarmNo);
	    //기업으로부터 온 인재제안 가져오기
	    List<AlarmVO> talentAlarm = recruitCheckService.getTalentList(loginUser);
	    
	    model.addAttribute("talentAlarmList", talentAlarm);

...
public class CustomUser extends User{
	...
	public CustomUser(AccountVO account) {
		...
		super(account.getAccountId(), account.getAccountPswd(), account.getAuthList().stream().map(auth -> new SimpleGrantedAuthority(auth.getAuth())).collect(Collectors.toList()));

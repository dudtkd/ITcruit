...
//		goPage = "codingtest/entCodingtestList";	
	...
	public ResponseEntity<String> entcodingSolve(
		...
		log.info("entcodingSolve왔어>>>>>>>>>>>>>>>" + entCodingtestVO);
		...
		//컴파일할 code에 입력받아온 코드를 입력
		codingtestVO.setCode(entCodingtestVO.getQstnAnsr());
		//기업코테면 flag를 true로 설정
		codingtestVO.setEntTest(true);
        String result = codingtestService.doCompile(codingtestVO);
        log.info("기업코테 결과>>>>>>>>>>>>>>>" + result);
        
        if(result.equals("통과")) entCodingtestVO.setPassYn("Y");
